<div id="topbar" class="d-flex align-items-center {{ Helper::GeneralSiteSettings('style_header') ? 'fixed-top' : '' }} {{ Helper::GeneralSiteSettings('style_bg_type') ? 'topbar-no-bg' : 'header-bg' }}">
    <div class="container d-flex justify-content-between">
        <div class="contact-info d-flex align-items-center">
            @if (Helper::GeneralSiteSettings('contact_t5') != '')
                <span class="{{ Helper::GeneralWebmasterSettings('header_search_status') && Helper::GeneralSiteSettings('style_change') ? 'd-none d-lg-block' : '' }}">
        ...
        <div class="controls d-flex align-items-center">
            @if (Helper::GeneralWebmasterSettings('header_search_status'))
                <a class="header-search-btn" href="#"><i class="fa fa-search"></i> <span>{{ __('backend.search') }}</span></a>
                <div id="header-search-box">
                    ...
                    <form method="GET" action="{{ Helper::sectionURL(1) }}" class="header-form-search">
                        <input type="search" autocomplete="off" name="search_word" value="" required maxlength="50" placeholder="{{ __('backend.typeToSearch') }}" />

                        <button type="submit" class="btn btn-lg btn-theme"><i class="fas fa-search"></i> {{ __('backend.search') }}</button>
                    </form>
                </div>
                @push('after-scripts')
                    <script>
            ...
            @if (Helper::GeneralSiteSettings('style_change'))
                <div class="appearance-toggle">
                    ...
                    <label for="appearance-toggle-checkbox" class="checkbox-label">
                        <i class="fas fa-moon"></i> <small id="appearance-toggle-title" class="title">{{ __('backend.themes1') }}</small>
            ...
            @if (Helper::GeneralWebmasterSettings('dashboard_link_status'))
                @if (Auth::check())
                    <div class="dropdown header-dropdown d-none d-sm-block">
                        <button class="btn dropdown-toggle" type="button" id="dropdownDashboardBtn" data-bs-toggle="dropdown" aria-expanded="false">
                            <i class="fa fa-user"></i> {{ Auth::user()->name }}
                        </button>
                        <ul class="dropdown-menu" aria-labelledby="dropdownDashboardBtn">
                            <li><a class="dropdown-item" href="{{ route('adminHome') }}"><i class="fa fa-cog"></i> {{ __('frontend.dashboard') }}</a></li>
                            @if (Auth::user()->permissions == 0 || Auth::user()->permissions == 1)
                                <a class="dropdown-item" href="{{ route('usersEdit', Auth::user()->id) }}"> <i class="fa fa-user"></i> {{ __('backend.profile') }}</a>
                            @endif
                            @if (Helper::GeneralWebmasterSettings('inbox_status'))
                                ...
                                @endif
                            @endif
                            <a class="dropdown-item" href="{{ route('adminLogout') }}"><i class="fa fa-sign-out"></i> {{ __('backend.logout') }}</a>
                        </ul>
                    </div>
                @else
                    <span class="d-none d-sm-block dashboard_Link">
                        <a href="{{ route('adminHome') }}" target="_blank"><i class="fa fa-cog"></i> {{ __('frontend.dashboard') }}
                        </a>
                    </span>
                @endif
            @endif
            @if (count(Helper::languagesList()) > 1)
                <div class="dropdown header-dropdown">
                    <button class="btn dropdown-toggle" type="button" id="dropdownLangBtn" data-bs-toggle="dropdown" aria-expanded="false">
                        @if (@Helper::currentLanguage()->icon != '')
                            <img src="{{ asset('assets/dashboard/images/flags/' . @Helper::currentLanguage()->icon . '.svg') }}" alt="{{ @Helper::currentLanguage()->code }}" loading="lazy">
                        @endif
                        {{ @Helper::currentLanguage()->title }}
                    </button>
                    <ul class="dropdown-menu" aria-labelledby="dropdownLangBtn">
                        @foreach (Helper::languagesList() as $ActiveLanguage)
                            <a href="{{ Helper::languageURL($ActiveLanguage->code, @$page_type, @$page_id) }}" class="dropdown-item">
                                @if ($ActiveLanguage->icon != '')
                                    <img src="{{ asset('assets/dashboard/images/flags/' . $ActiveLanguage->icon . '.svg') }}" alt=" {{ $ActiveLanguage->code }}" loading="lazy">
                                @endif
