                            ...
                            <div class="col-sm-9 p-a-0">
                                ...
                                <div class="p-y">
                                    @if(Session::has('EditContactsGroup'))
                                        <form method="POST" action="{{ route("contactsUpdateGroup",Session::get('EditContactsGroup')->id) }}" class="dashboard-form">
                                            @csrf
                                            <div class="input-group input-group-sm">
                                                <input type="text" autocomplete="off" name="name" id="edit_name" value="{{ Session::get('EditContactsGroup')->name }}" required placeholder="{{ __('backend.newGroup') }}" class="form-control"/>
                ...
                <button class="btn btn-default b-a no-shadow" type="submit">{!! __('backend.save') !!}</button>
                                    ...
                                    @else
                                        @if(@Auth::user()->permissionsGroup->add_status)
                                            <form method="POST" action="{{ route("contactsStoreGroup") }}" class="dashboard-form">
                                                @csrf
                                                <div class="input-group input-group-sm">
                                                    <input type="text" autocomplete="off" name="name" id="name" value="" required placeholder="{{ __('backend.newGroup') }}" class="form-control"/>
                                                    <span class="input-group-btn">
