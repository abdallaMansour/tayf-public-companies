...
@endif
@if(count($WebmasterSections->allCustomFields)>0)
    <form method="POST" action="{{ route("webmasterFieldsUpdateAll",$WebmasterSections->id) }}" class="dashboard-form">
        @csrf
        <div>
                    ...
                    <tr>
                        <td class="{!! $customField->css_class !!} dker"><label class="ui-check m-a-0">
                                <input type="checkbox" name="ids[]"
                                    ...
                                    class="dark-white"></i>
                                <input type="hidden" name="row_ids[]" value="{{ $customField->id }}" class="form-control row_no">
                            </label>
                        </td>

                        <td class="text-center">{{ $customField->id }}</td>
                        <td class="h6 {!! $customField->css_class !!}">
                            <input type="text" name="row_no_{{ $customField->id }}" value="{{ $customField->row_no }}" class="form-control row_no" autocomplete="off">
                            {!! $field_title  !!}</td>
