@if($WebmasterSection->code_status)
    <div class="tab-pane  {{ $tab_3 }}" id="tab_code">

        <div class="box-body p-a-2">
            <form method="POST" action="{{ route("categoriesCodeUpdate",["webmasterId"=>$WebmasterSection->id,"id"=>$Sections->id]) }}" class="dashboard-form">
                @csrf
                <div>
                    <div class="form-group">
                        <h6>{!!  __('backend.customCSS') !!}</h6>
                        <div dir="ltr">
                            <textarea name="css_code" id="css_code" class="form-control" dir="ltr" rows="10">{{ @$Sections->css_code }}</textarea>
                        </div>
                    </div>
                    <div class="form-group">
...
<script>
...
</script>">{{ @$Sections->js_code }}</textarea>
                        </div>
                    </div>
                    <div class="form-group">
                        <h6>{!!  __('backend.customCodeOnBody') !!}</h6>
                        <div dir="ltr">
                            <textarea name="body_code" id="body_code" class="form-control" dir="ltr" rows="10">{{ @$Sections->body_code }}</textarea>
                        </div>
                    </div>
                </div>
                <div class="form-group">
