...
class WebmailsController extends Controller
{
    ...
    public function index($group_id = null, $wid = null, $stat = null, $contact_email = null)
    {
        ...
        // Check Permissions
        if (!@Auth::user()->permissionsGroup->inbox_status || !Helper::GeneralWebmasterSettings("inbox_status")) {
    ...
    public function search(Request $request)
    {
        ...
        if (@Auth::user()->permissionsGroup->view_status) {
            ...
            if ($request->q != "") {
                ...
                return redirect()->action([WebmailsController::class, 'index']);
            }
            //Count of unread
            $WaitWebmailsCount = Webmail::where('created_by', '=', Auth::user()->id)->where('status', '=',
                '0')->count();
    ...
    public function storeGroup(Request $request)
    {
        ...
        return redirect()->action([WebmailsController::class, 'index'])->with('doneMessage', __('backend.saveDone'));
    }
    ...
    public function store(Request $request)
    {
            ...
            if (!empty($files)) {
                ...
                if ($file_count > 0) {
                    foreach ($files as $file) {
                        $validator = Validator::make(array('file' => $file), array('file' => 'file|mimes:'.config('filesystems.allowed_file_types')));
                        if ($validator->passes()) {
                            $FileInfo = FileHelper::uploadFile($file, $this->uploadPath, 1);
        ...
        if ($request->btn_clicked != "draft") {
            ...
            return redirect()->action([WebmailsController::class, 'index'])->with('doneMessage',
                __('backend.messageSentSuccessfully'));
        }
    ...
    public function edit($id)
    {
        ...
        if (!empty($WebmailToEdit)) {
            ...
            $WebmailToEdit->save();
            return redirect()->action([WebmailsController::class, 'index'],
                ['group_id' => $WebmailToEdit->group_id])->with('WebmailToEdit',
                $WebmailToEdit);
        } else {
            return redirect()->action([WebmailsController::class, 'index']);
        }
    ...
    public function editGroup($id)
    {
        ...
        if (!empty($EditWebmailsGroup)) {
            return redirect()->action([WebmailsController::class, 'index'])->with('EditWebmailsGroup',
                $EditWebmailsGroup);
        } else {
            return redirect()->action([WebmailsController::class, 'index']);
        }
    ...
    public function destroy($id)
    {
        ...
        Cache::forget('_Loader_Webmails');
        return redirect()->action([WebmailsController::class, 'index'])->with('doneMessage', __('backend.deleteDone'));
    }
    ...
    public function updateAll(Request $request)
    {
            ...
            if ($request->action == "read") {
                ...
                foreach ($WebmailsFiles as $WebmailsFile) {
                    if ($WebmailsFile->file != "") {
                        FileHelper::deleteFile($this->uploadPath."/".$WebmailsFile->file);
                    }
                }
        ...
        Cache::forget('_Loader_Webmails');
        return redirect()->action([WebmailsController::class, 'index'])->with('doneMessage2', __('backend.saveDone'));
    }
